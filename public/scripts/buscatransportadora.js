  ...
  if (transportadoraInput && suggestionsList) {
      transportadoraInput.addEventListener("input", async () => {
          ...
          try {
              ...
              const response = await fetch(`/../../routes/api/transportadoras?q=${encodeURIComponent(query)}`);
      ...
      document.addEventListener("click", (event) => {
          if (!transportadoraInput.contains(event.target) && !suggestionsList.contains(event.target)) {
              suggestionsList.innerHTML = "";
              suggestionsList.classList.add("hidden");
          }
      });
